namespace HtmlParserMajestic
{
    ...
    public class HtmlEntities
    {
        ...
        /// </summary>
        private readonly int iMinEntityLen;

        /// <summary>
        /// Internal heuristics for entiries: these will be set to min and max string lengths of known HTML entities
        /// </summary>
        private readonly int iMaxEntityLen;

        private static readonly int iAllMinEntityLen;

        private static readonly int iAllMaxEntityLen;
        ...
        {
            this.oEntities = InitEntities(ref this.iMinEntityLen, ref this.iMaxEntityLen, out sEntityReverseLookup);
            this.bMiniEntities = false;
        }
				...
				*/
                while (iCurPos < iDataLength)
                {
                    ...
                    if (cChar == ';' || (bCharCode && !bCharCodeHex && !(cChar >= '0' && cChar <= '9')))
                    {
                        ...
                        {
                            object oChar = this.oEntities.GetLikelyPresentValue(bHTML[iFrom], bHTML[iFrom + 1]);
                        ...
                        if (bCharCode && iEntLen > 0 && !bCharCodeHex)
                        {
                            ...
                            return (char)this.ParseUInt(bHTML, iFrom, iEntLen);
                        }
    ...
    // we want to avoid exceptions if possible as they are very slow
                        ...
                        if (iEntLen >= this.iMinEntityLen && iEntLen <= this.iMaxEntityLen)
                        {
                            object oChar = this.oEntities.GetLikelyPresentValue(sEntity);
...
#endif
        {
            ...
            var oSB = new StringBuilder(sData.Length);
                        ...
                        for (; j < sData.Length; j++)
                        {
                                ...
                                || (bCharCode && bLastChar))
                            {
                                ...
                                if (!bCharCode && iEntLen > 1)
                                {
                                    ...
                                    if (i + 2 < sData.Length)
                                    {
                                        object oChar = oAllEntities.GetLikelyPresentValue(
                                            (byte)sData[i + 1], (byte)sData[i + 2]);
            ...
            {
                Console.WriteLine("Entity parsing exception: " + oEx);
        ...
        /// </summary>
        private static readonly uint[] iDecMultipliers = {
        ...
        private uint ParseUInt(byte[] bBuf, int iFrom, int iLength)
        {
            uint iNum = 0;
        ...
        /// </summary>
        private static FastHash InitEntities(
            ref int iMinEntityLen, ref int iMaxEntityLen, out string[] sEntityReverseLookup)
        {
            var oEntities = new FastHash();
        ...
        internal string ChangeToEntities(string sLine, int iFrom, bool bChangeDangerousCharsOnly)
        {
            ...
            for (int i = iFrom; i < sLine.Length; i++)
            {
                ...
                {
                    if (sEntityReverseLookup[cChar] != null)
                    {
                        ...
                        {
                            oSB.Append("&");
                            oSB.Append(sEntityReverseLookup[cChar]);
                            oSB.Append(";");
                        }
                        else
                        {
        ...
        {
            this.oEntities = new FastHash();

            this.oEntities.Add("nbsp", 32);
            this.bMiniEntities = true;
        }
